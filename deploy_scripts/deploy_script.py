...
ignore_pattern = r'deploy_scripts|.git|.idea|TestScripts'
...
def get_files_to_copy(dir_name):
    ...
    for root, dirs, files in os.walk(dir_name):
        files_to_copy_list += [os.path.join(root, name) for name in files if (bool(re.search(files_to_copy_pattern, os.path.join(root, name))) and
...
for root, dirs, files in os.walk(deploy_path + '\\regulargrid3\\', topdown=False):
    for file in files:
        curpath = os.path.join(root, file)
        os.remove(curpath)
...
for file in files_to_copy:
    target_path = file.replace(root_path, deploy_path + '\\regulargrid3\\')
    if not os.path.exists(os.path.dirname(target_path)):
        os.mkdir(os.path.dirname(target_path))
    shutil.copy(file, target_path)
