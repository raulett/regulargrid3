...
from ..utils.rectangularpoint import RectangularPoint
...
class RectangularPointsTool():
        ...
        def showDialog(self):
            pydevd_pycharm.settrace('localhost', port=5842, stdoutToServer=True, stderrToServer=True)
            currLayer = self.canvas.currentLayer()
            selectedFeat = currLayer.selectedFeatures()
            points = selectedFeat[0].geometry().asMultiPolyline()
